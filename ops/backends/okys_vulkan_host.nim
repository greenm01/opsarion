...
const okysRoot =
  currentSourcePath().parentDir().parentDir().parentDir().parentDir() / "okys"
const H = "okys.h"
...
type
  OKYplatformHostObj {.importc: "OKYplatformHost", header: H, incompleteStruct.} = object
...
proc okyPlatformHostResize(
  host: ptr OKYplatformHostObj, width, height: uint32
): cint {.importc, header: H.}

proc okyPlatformHostBeginFrame(
  host: ptr OKYplatformHostObj, frame: ptr OpsVkFrame
): cint {.importc, header: H.}

proc okyPlatformHostPresent(
  host: ptr OKYplatformHostObj, frame: ptr OpsVkFrame
): cint {.importc, header: H.}

proc okyPlatformHostVulkanInstance(
  host: ptr OKYplatformHostObj
): pointer {.importc, header: H.}

proc okyPlatformHostVulkanPhysicalDevice(
  host: ptr OKYplatformHostObj
): pointer {.importc, header: H.}

proc okyPlatformHostVulkanDevice(
  host: ptr OKYplatformHostObj
): pointer {.importc, header: H.}

proc okyPlatformHostVulkanQueue(
  host: ptr OKYplatformHostObj
): pointer {.importc, header: H.}

proc okyPlatformHostVulkanQueueFamilyIndex(
  host: ptr OKYplatformHostObj
): uint32 {.importc, header: H.}

proc okyPlatformHostColorFormatCode(
  host: ptr OKYplatformHostObj
): cint {.importc, header: H.}
...
proc initOkysVulkanHostWithSurface*(
  ...
  if handle.kind != kwskWayland:
    raise newException(
      CatchableError, "okys native Vulkan host currently supports Wayland surfaces"
    )
  h.raw =
    okyPlatformHostCreateWayland(handle.wlDisplay, handle.wlSurface, width, height)
  if h.raw.isNil:
...
proc presentOkysVulkanFrame*(h: var OkysVulkanHost, frame: var OpsVkFrame): bool =
  if h.raw.isNil:
    return false
